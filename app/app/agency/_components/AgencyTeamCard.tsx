          ...
          <DialogContent className="sm:max-w-[420px] rounded-2xl p-6">
            ...
            <form action={formAction} className="space-y-5">
              ...
              <Button
                type="submit"
                disabled={isPending}
                className="w-full h-10 bg-slate-900 text-white hover:bg-slate-800 shadow-md shadow-slate-900/10 transition-all"
              >
                {isPending ? (
        ...
        <div className="bg-slate-50/50 border-t border-slate-100 px-7 py-5">
          ...
          <div className="space-y-2">
            {invites.slice(0, 3).map((invite: any) => (
              <div
                key={invite.id}
                className="flex items-center gap-3 rounded-xl border border-slate-200/60 bg-white p-3 shadow-sm"
              >
